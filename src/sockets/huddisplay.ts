...
const displaySocket = new JavaAdapter(
  WebSocketClient,
  {
    onMessage(message: string): void {
      const { product_id, price, open_24h }: WebSocketData =
        JSON.parse(message);
      ...
      liveDisplay
        .setLine(
          0,
          new DisplayLine(product_id).setAlign(DisplayHandler.Align.CENTER)
        )
        .setLine(1, `Open: $${addCommas(Number(open_24h))}`)
        .setLine(
          2,
          `Price: ${Number(price) >= lastPrice ? "§a" : "§c"}$${addCommas(
            Number(price)
          )}`
        )
        .setLine(
          3,
          `24h Change: ${currentSymbol} ${addCommas(
            priceRange,
            findDecimalPlaces(Number(price))
          )} (${pricePercentDiff.toFixed(4)}%)`
        );
...
register("step", () => {
  if (!Settings.sendDisplayRequest) return;
...
register("step", () => {
  if (liveGui.isOpen()) {
    if (displaySocket.isOpen()) displaySocket.closeBlocking();

    liveDisplay
      .setLine(
        0,
        new DisplayLine("Sample").setAlign(DisplayHandler.Align.CENTER)
      )
      .setLine(1, `Open:`)
      .setLine(2, `Price:`)
  ...
  liveDisplay.setShouldRender(
    (Settings.toggleDailyDisplay && displaySocket.isOpen()) || liveGui.isOpen()
  );
}).setFps(20);
