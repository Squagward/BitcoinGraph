...
const displaySocket = new JavaAdapter(WebSocketClient, {
    onMessage(message) {
        const { type, product_id, price, open_24h, volume_24h, low_24h, high_24h, volume_30d, time, last_size } = JSON.parse(message);
        if (type !== "ticker")
            return;
        ...
        lines[0].setText(`Price: ${getPriceChangeColor(Number(price), lastPrice)}$${addCommas(Number(price))}`);
