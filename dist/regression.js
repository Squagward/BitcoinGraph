const linearRegression = (data) => {
    ...
    const SUM_X2 = data.reduce((a, b) => Math.pow(b.x, 2) + a, 0);
    const SUM_Y2 = data.reduce((a, b) => Math.pow(b.y, 2) + a, 0);
...
const quadraticRegression = (data) => {
    ...
    const SUM_Y = data.reduce((a, b) => b.y + a, 0);
    const SUM_X2 = data.reduce((a, b) => Math.pow(b.x, 2) + a, 0);
    const SUM_X3 = data.reduce((a, b) => Math.pow(b.x, 3) + a, 0);
    const SUM_X4 = data.reduce((a, b) => Math.pow(b.x, 4) + a, 0);
    ...
    const SUM_X2Y = data.reduce((a, b) => a + Math.pow(b.x, 2) * b.y, 0) - (SUM_X2 * SUM_Y) / N;
